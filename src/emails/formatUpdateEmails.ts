...
export const formatUpdateEmail = (id: string, emailInfo: updateEmail) => {
  ...
  const canadaPostTrackUrl = `https://www.canadapost-postescanada.ca/track-reperage/en#/search?searchFor=${encodeURIComponent(tracking)}`;
  const title = isShipped ? "Your Order Has Shipped!" : "Order Update";
  const message = isShipped
    ? `Great news — your <strong>Kiloboy</strong> order is on its way. <a href="${canadaPostTrackUrl}" style="color:#333;">Track your package at Canada Post</a> using the tracking number below.`
          ...
          <td>
            ${
              isShipped && emailInfo.tracking_number
                ? `<a href="${canadaPostTrackUrl}" style="color:#333;">${tracking}</a>`
                : tracking
            }
          </td>
        </tr>
